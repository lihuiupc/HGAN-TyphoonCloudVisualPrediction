...
class DiscriminatorModel:
    ...
    def define_graph(self):
        ...
        with tf.name_scope('discriminator'):
            ...
            for scale_num in xrange(self.num_scale_nets):
                with tf.name_scope('scale_net_' + str(scale_num)):
                    ...
                    self.scale_nets.append(DScaleModel(scale_num,
                                                       int(self.height * scale_factor/2),###(/2)
                                                       int(self.width * scale_factor/2),
            ...
            #####self.G_x_history = tf.placeholder(tf.float32, shape=[None, self.height, self.width, (3 * HIST_LEN)], name='R_x_history')
    ...
    def build_feed_dict(self, input_frames, gt_output_frames, generator):
        ...
        for scale_num in xrange(self.num_scale_nets):
            ...
            if g_scale_preds[scale_num].shape[1]%2 != 0:
               g_scale_preds[scale_num] = g_scale_preds[scale_num][:,:-1,:,:]
            if g_scale_preds[scale_num].shape[2]%2 != 0:
               g_scale_preds[scale_num] = g_scale_preds[scale_num][:,:,:-1,:]
################
            local_g_preds = np.empty([batch_size*4, scale_net.height, scale_net.width, 3])

            for i in range(batch_size):
                   local_g_preds[4*i] = g_scale_preds[scale_num][i,:scale_net.height,:scale_net.width,:]
                   local_g_preds[4*i+1] = g_scale_preds[scale_num][i,:scale_net.height,scale_net.width:,:]
                   local_g_preds[4*i+2] = g_scale_preds[scale_num][i,scale_net.height:,:scale_net.width,:]
                   local_g_preds[4*i+3] = g_scale_preds[scale_num][i,scale_net.height:,scale_net.width:,:]
	    local_g_scale_preds.append(local_g_preds)
#################
            # resize gt_output_frames
            scaled_gt_output_frames = np.empty([batch_size, 2*scale_net.height, 2*scale_net.width, 3])
            local_gt_output_frames = np.empty([batch_size*4, scale_net.height, scale_net.width, 3])##########
            for i, img in enumerate(gt_output_frames):
                ...
                resized_frame = resize(sknorm_img, [2*scale_net.height, 2*scale_net.width, 3])
                scaled_gt_output_frames[i] = (resized_frame - 0.5) * 2
################
                local_gt_output_frames[4*i] = scaled_gt_output_frames[i,:scale_net.height,:scale_net.width,:]
		local_gt_output_frames[4*i+1] = scaled_gt_output_frames[i,:scale_net.height,scale_net.width:,:]
		local_gt_output_frames[4*i+2] = scaled_gt_output_frames[i,scale_net.height:,:scale_net.width,:]
		local_gt_output_frames[4*i+3] = scaled_gt_output_frames[i,scale_net.height:,scale_net.width:,:]
###############
            ...
            ###                                      scaled_gt_output_frames])###[2b,h,w,c]
	    scaled_input_frames = np.concatenate([local_g_scale_preds[scale_num],
                                                  local_gt_output_frames])###[8b,h,w,c]
        ...
        feed_dict[self.labels] = np.concatenate([np.zeros([4*batch_size, 1]),###batch_size
                                                 np.ones([4*batch_size, 1])])###[8b,1]

        return feed_dict

    def train_step(self, batch, buff, generator):####(buff)
        """
        ...
        return c_feed_dict#######(global_step,c_feed_dict)
